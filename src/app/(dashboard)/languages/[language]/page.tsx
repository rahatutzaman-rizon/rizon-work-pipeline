...
import { ArrowLeft, Languages, ChevronRight, BookOpen, CheckSquare, Plus, X, Calendar, ListPlus, Upload, FileText, Image as ImageIcon } from 'lucide-react';
import {
  VocabSetItem,
  NoteItem,
  TaskItem,
...
import { LanguageAiAssistantAgent } from '@/components/languages/LanguageAiAssistantAgent';
import { UploadedFileViewerSection } from '@/components/notes/UploadedFileViewerSection';

export default function LanguageSetsPage({ params }: { params: Promise<{ language: string }> }) {
  const resolvedParams = use(params);
  const language = resolvedParams.language as 'english' | 'spanish';

  const [sets, setSets] = useState<VocabSetItem[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    const [sList, nList, tList] = await Promise.all([
      fetchVocabSets(language),
      fetchNotesBySubject('languages', language),
      fetchTasksBySubject('languages', language),
    ]);
    setSets(sList);
    setNotes(nList);
    setTasks(tList);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      <Link href="/languages" className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-amber-700">
        <ArrowLeft className="w-4 h-4 text-amber-600" />
        <span>Back to Languages Hub</span>
      </Link>

      <div className="rounded-3xl bg-gradient-to-r from-amber-700 via-orange-800 to-amber-900 p-8 text-white shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <span className="px-3 py-1 rounded-full text-[10px] font-black bg-amber-300 text-amber-950 uppercase">
            Spoken & Written {isEnglish ? 'English' : 'Spanish'}
          </span>
          <h1 className="text-3xl font-black mt-2">
            {isEnglish ? 'English Vocabulary & Speaking' : 'Spanish Vocabulary & Speaking'}
          </h1>
          <p className="text-xs text-amber-100 mt-1">Interactive Flashcards, Speech AI & Notes</p>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 bg-amber-300 hover:bg-amber-400 text-amber-950 rounded-xl text-xs font-black shadow-md flex items-center gap-1.5"
          >
      ...
      {/* AI SPOKEN & WRITING ASSISTANT AGENT */}
      <LanguageAiAssistantAgent language={language} />

      {/* CREATE NOTE FORM WITH PDF & IMAGE FILE UPLOAD */}
      {showCreateNote && (
        <form onSubmit={handleCreateNote} className="p-6 bg-white rounded-3xl border-2 border-amber-500 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-900">Create Note & Attach Files for {isEnglish ? 'English' : 'Spanish'}</h3>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="px-5 py-2 bg-amber-600 text-white rounded-xl text-xs font-extrabold">
              Save Note & Files
            </button>
          </div>
        </form>
      ...
      {/* CREATE DETAILED TASK FORM */}
      {showCreateTask && (
        <form onSubmit={handleCreateTask} className="p-6 bg-white rounded-3xl border-2 border-amber-500 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-900">Create Detailed Task for {isEnglish ? 'English' : 'Spanish'}</h3>
          ...
          <input
            type="text"
            placeholder="Task Title (e.g. Master Daily Conversation Flashcards)..."
            value={taskTitle}
          ...
          />
          <textarea
            placeholder="Detailed Description / Spoken Practice Instructions..."
            value={taskDesc}
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
          ...
          <span>Vocabulary Flashcard Sets</span>
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {sets.map((set) => (
            <Link
              key={set.id}
              href={`/languages/${language}/${set.slug}`}
              className="p-6 rounded-3xl bg-white border border-slate-200 hover:border-amber-400 shadow-xs hover:shadow-lg transition-all group flex items-center justify-between"
            >
              <div>
      ...
      <div className="space-y-4 pt-4">
        <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
          ...
          <span>{isEnglish ? 'English' : 'Spanish'} Practice Notes ({notes.length})</span>
        </h2>
        {notes.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {notes.map((n) => (
              <div
                ...
                className="p-5 rounded-2xl bg-white border border-slate-200 hover:border-amber-500 shadow-xs hover:shadow-md cursor-pointer space-y-2"
              >
                  ...
                  <div className="flex items-center gap-2 flex-wrap pt-1">
                    {n.attachments.map((att, idx) => (
                      <span key={idx} className="px-2 py-1 rounded-md text-[10px] font-extrabold bg-slate-100 text-slate-700 border border-slate-200 inline-flex items-center gap-1">
                        {att.type === 'pdf' ? <FileText className="w-3 h-3 text-red-500" /> : <ImageIcon className="w-3 h-3 text-amber-600" />}
          ...
          <div className="p-8 text-center bg-white border border-slate-200 rounded-2xl text-xs font-bold text-slate-500">
            No notes added for {isEnglish ? 'English' : 'Spanish'} yet.
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
          ...
          <span>{isEnglish ? 'English' : 'Spanish'} Tasks ({tasks.length})</span>
        </h2>
        {tasks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tasks.map((t) => (
              <div
                ...
                className="p-4 rounded-2xl bg-white border border-slate-200 hover:border-amber-500 shadow-xs hover:shadow-md cursor-pointer flex justify-between items-center"
              >
                <div>
                ...
                <span className="text-xs font-bold text-amber-600">View Detail</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="p-8 text-center bg-white border border-slate-200 rounded-2xl text-xs font-bold text-slate-500">
            No tasks added for {isEnglish ? 'English' : 'Spanish'} yet.
          </div>
        )}
      </div>
