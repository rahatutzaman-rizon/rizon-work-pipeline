...
export default function SoftwareAiWorkspacePage({ params }: { params: Promise<{ subjectSlug: string }> }) {
  ...
  const loadData = async () => {
    setLoading(true);
    const sub = await fetchSubjectBySlug('software-ai', subjectSlug);
    setSubject(sub);

    if (sub) {
      const targetId = sub.id || subjectSlug;
      const [nList, tList] = await Promise.all([
        fetchNotesBySubject('software-ai', targetId),
        fetchTasksBySubject('software-ai', targetId),
      ]);
      setNotes(nList);
      setTasks(tList);
    }
  ...
  if (loading) return <div className="p-12 text-center text-slate-500 font-bold">Loading Software AI Workspace...</div>;
  if (!subject) return <div className="p-12 text-center font-bold">Subject Not Found</div>;

  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      <Link href="/software-ai" className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-purple-700">
        <ArrowLeft className="w-4 h-4 text-purple-600" />
        <span>Back to Software & AI Hub</span>
      </Link>

      <div className="rounded-3xl bg-gradient-to-r from-purple-900 via-violet-950 to-slate-950 p-8 text-white shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <span className="px-3 py-1 rounded-full text-[10px] font-black bg-fuchsia-400 text-purple-950 uppercase">
            Software Engineering & AI Architecture
          </span>
          <h1 className="text-3xl font-black mt-2">{subject.name_bn || subject.name_en}</h1>
          <p className="text-xs text-purple-200 mt-1">{subject.name_en}</p>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 bg-fuchsia-400 hover:bg-fuchsia-500 text-purple-950 rounded-xl text-xs font-black shadow-md flex items-center gap-1.5"
          >
      ...
      {showCreateNote && (
        <form onSubmit={handleCreateNote} className="p-6 bg-white rounded-3xl border-2 border-purple-500 shadow-xl space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="px-5 py-2 bg-purple-600 text-white rounded-xl text-xs font-extrabold">
              Save Note & Files
            </button>
          </div>
        </form>
      ...
      {showCreateTask && (
        <form onSubmit={handleCreateTask} className="p-6 bg-white rounded-3xl border-2 border-purple-500 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {notes.map((n) => (
              <div
                ...
                className="p-5 rounded-2xl bg-white border border-slate-200 hover:border-purple-500 shadow-xs hover:shadow-md cursor-pointer space-y-2"
              >
                  ...
                  <div className="flex items-center gap-2 flex-wrap pt-1">
                    {n.attachments.map((att, idx) => (
                      <span key={idx} className="px-2 py-1 rounded-md text-[10px] font-extrabold bg-slate-100 text-slate-700 border border-slate-200 inline-flex items-center gap-1">
                        {att.type === 'pdf' ? <FileText className="w-3 h-3 text-red-500" /> : <ImageIcon className="w-3 h-3 text-purple-600" />}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tasks.map((t) => (
              <div
                ...
                className="p-4 rounded-2xl bg-white border border-slate-200 hover:border-purple-500 shadow-xs hover:shadow-md cursor-pointer flex justify-between items-center"
              >
                <div>
                ...
                <span className="text-xs font-bold text-purple-600">View Detail</span>
              </div>
            ))}
          </div>
        ) : (
